...
%\textbullet $X_n$ indep., $\mathbb{E}(X_n){=}0$, $\mathbb{E}(X^2_n){<}\infty$ $\Rightarrow$ 
%$\!P\left(\max\limits_{k{\leq} n}|S_k|\geq\epsilon\right) {\leq} \frac{\mathrm{Var}(S_n)}{\epsilon^2}$

\textbullet $X_n$ indep., $\mathbb{E}(X_n){=}0$, $\sum_{n=1}^\infty\frac{1}{n^2}\mathrm{Var}(X_n)<\infty \Rightarrow \frac{S_n}{n}\to 0$ a.\,s.\ %\vspace{0.5em}
...
%(for r.v.\ $(X_n)$ this means $P_n{=}P_{X_n}$, $P{=}P_X$ and $X_n\to X$ weakly)
...
%it follows: $P_{X_n}\to P_X$ weakly $\Rightarrow \varphi_{X_n}\to\varphi_X$

%\textbf{Helly's Theorem} $F_n$ PDFs $\exists F,k_n$ s.t.\ $F_{k_n}(x) {\to} F(x)$ at cont.\ $x$
...
%$\varphi$ is char.\ function of Borel measure $P$ on $\mathbb{R}$ if $\varphi(t)=\int\mathrm{e}^{\i tx}dP(x)$

%$(P_n)$ tight, subsequence $\to \!P$, $\varphi_n(u){\to}\varphi(u) \Rightarrow P_n{\to} P$ weakly
...
% falejandro's book
\subsection[Modern Causal Inference]{Modern Causal Inference \citep{schuler2024introduction}}

\begin{multicols}{2}

\paragraph{Inference \& Statistics} ask clear questions

\textbf{descriptive} data $\mathbb{P}_n \to$ estimator $\hat{\psi}$ $\to$ estimate $\hat{\psi}(\mathbb{P}_n)$

\textbf{inferential} statistical model (set of possible worlds) + 
estimand (real world property) $\to$ 
inference (link estimate to real world \& quantify uncertainty) via the sampling distribution

in an RCT: linear regression coefficient equals the ATE

\paragraph{Causality \& Identification} link $P$ to causal world $P^*$

\textbf{identification} find $\psi(P)=\psi^*(P^*)$   (= causal estimand)
...
\subsection{Incremental Effects}

\begin{multicols}{2}

\paragraph{Binary Data} based on propensity score \citep{kennedy2019nonparametric}

\begin{itemize}[leftmargin=*, itemsep=0em, topsep=0pt, partopsep=0pt,parsep=0pt]
\item[$+$] no positivity assumption, no parametric assumptions
\item[$+$] longitudinal effects in single curve
\item[$-$] descriptive rather than prescriptive (stochastic)
\end{itemize}

\textbf{intervention} with conditional distribution where $\delta{\in}(0,\infty)$:

 $q_t(h_t)=\frac{\delta\pi_t(h_t)}{\delta\pi_t(h_t)+1-\pi_t(h_t)}$
\hfill therefore \hfill
$\delta=\frac{\text{odds}_q(A_t=1|H_t=h_t)}{\text{odds}_\pi(A_t=1|H_t=h_t)}$

\textbf{identification} requires only consistency \& exchangeability
$$\mathbb{E}\left(Y^{Q(\delta)}\right){=}\!\sum_{\bar{a}_T\in\mathcal{A}^T}\int_\chi \!\mu(h_t,a_t)\prod_{t=1}^T\frac{a_t\delta\pi_t(h_t)+(1{-}a_t)(1{-}\pi_t(h_t))}{\delta\pi_t(h_t)+1-\pi_t(h_t)}$$

\vspace{-0.5em}


\hfill $d\mathbb{P}(x_t|h_{t-1},a_{t-1})$

 \hspace{4.1em} $\overset{T{=}1}{=}\mathbb{E}\left[\frac{\delta\pi(X)\mu(X,1)+(1-\pi(X))\mu(X,0)}{\delta\pi(X)+1-\pi(X)}\right]$


\paragraph{Continuous Data} \citep{schindl2024incremental}
\vspace{20em}
