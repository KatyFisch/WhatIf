...
\paragraph{Real Numbers}  \textit{triangle inequality} $|a+b| \leq |a| + |b|$
 ...
 \textbf{Cardinality:} $A\sim B$, if $\exists f:A\to B$, where $f$ is 1-1 and onto

 \textit{function} $f:A\to B$ mapping $f(x)=...$, domain = $A$, range $\subseteq B$
``1-1'' if $a_1{\neq} a_2 \Rightarrow f(a_1){\neq} f(a_2)$,
``onto'' if $\forall b{\in} B \, \exists a{\in} A: f(a){=}b$
...
\paragraph{Sequences} are functions with domain $\mathbb{N}$
 ...
 \textbf{Algebraic Limit Theorem:} if $(a_n){\to} a$, $(b_n){\to} b$, then $(ca_n) {\to} ca$, $(a_n {+} b_n){\to} a{+}b$, $(a_nb_n){\to} ab$, $(a_n/b_n){\to} ab$ for $b\neq 0$

 \textbf{Order Limit Theorem:} $a_n\geq 0 \, \forall n\in\mathbb{N} \Rightarrow a\geq 0$ ($\leq$ analogous), $\exists c\in\mathbb{R} c\leq b_n\forall\mathbb{N}\Rightarrow c\leq b$ ($\geq$ analogous)
 ...
 \textbf{Bolzano-Weierstrass Theorem:} all bounded sequences have a convergent subsequence
...
, \textit{geometric series} $\sum_{k=0}^\infty ar^k \overset{|r|<1}{=} \frac{1}{1-r}$

 \textbf{Convergence:} to $B$, if $(s_m)\to B$ with partial sums $s_m=\sum_{n=1}^m$
...
\newline 
$\sum_{n=1}^\infty |a_n|$ converges $\Rightarrow$ $\sum_{n=1}^\infty a_n$ too
...
\item\textit{Any open cover for $A$ has a finite subcover:} An open cover is a set of open sets $\left\{O_\lambda:\lambda\in\Lambda\right\}$ whose union contains $A$; a finite subcover is a finite subset that still covers $A$
 ...
 \textbf{Separation:} of $A$ and $B$ if $\bar{A}\cap B$ \& $A\cap \bar{B}$

 \textbf{Disconnection:} if $A=B\cup C$, with $B,C$ nonempty \& separated;
\newline
$E$ is connected $\Leftrightarrow$ all nonempty disjoint sets  $B,C$ s.t.\ $E=B\cup C$ have a convergent sequence with a limit in the other set $\Leftrightarrow$ whenever $a<c<b$ with $a,b\in E$, then $c \in E$

  \textbf{Baire's Theorem:} $\mathbb{R}$ cannot be written as the countable union of nowhere-dense sets; $E$ is nowhere-dense if $\bar{E}$ contains no nonempty open intervals

\paragraph{Functional Limits and Continuity}  \ \\

 \textbf{Functional Limit:} let $f:A\to\mathbb{R}$ and $c$ limit point of $A$
$\forall \epsilon > 0 \, \exists \delta>0$ s.t.\ $0<|x-c|<\delta$ (and $x\in A$) if follows $|f(x)-L|<\epsilon$, then $\lim_{x\to c} f(x)=L$
...
$\displaystyle\lim_{x\to c}\left[f(x) + g(x)\right]=L+M$; $\displaystyle\lim_{x\to c}\left[f(x)g(x)\displaystyle\right]=LM$; \,\,\, $\displaystyle\lim_{x\to c}f(x)/g(x)=L/M$ if $M\neq 0$
 ...
 \textbf{Continuity at $c$:} $\forall \epsilon>0\,\exists \delta>0$ s.t.\ whenever
 ...
 can also be expressed as: \newline $(x_n)\to c$ (with $x_n \in A$) $\Rightarrow f(x_n)\to f(c)$
  ...
  \textbf{Boundedness:} $f$ is bounded on its domain $A$ $\Leftrightarrow$ $f(A)$ is bounded; $f$ is bounded \textit{on $B$} $\Leftrightarrow$ $f(B)$ is bounded
...
\textbf{0/0: } let $a\in I$, $f,g$ continuous on $I$, diff'able on $I\setminus a$:  
...
\textbf{Continuity:}  let $(f_n)$ converges uniformly to $f$, if all $f_n$ are continuous at $c\in A$, then $f$ is in continuous at $c$
...
$$\mathrm{E}\left[Y^{a}\right] = \mathrm{E}\left[\frac{I(A=a)Y}{f\left[A|L\right]}\right]; W^A=\frac{1}{f\left[A|L\right]}; SW^A = \frac{f(A)}{f\left[A|L\right]}$$
 ...
 \textbf{stabilized weights $\boldsymbol{SW^A}$} should have an average of 1 (check!) $\rightarrow$ pseudo-population same size $\rightarrow$ CI width $\downarrow$
