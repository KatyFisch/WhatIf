...
if $\exists (e_n)$ in $X$ s.t.\ $\forall x \,\, \|x-(\alpha_1e_1+...+\alpha_ne_n)\|\overset{n\to\infty}{\to} 0$ then $e_n$ is called \textit{Schrauder basis} with $x{=}\sum_{k=1}^\infty\alpha_ke_k$; $\exists (e_n)\Rightarrow X$ separable
...
\textbf{Finite Dimensional Normed Spaces} every finite dimensional subspace $Y$ of a normed space $X$ is complete and closed in $X$;

in a finite dimensional vector space $X$ all norms are equivalent, meaning $\exists a,b>0$ s.t\ $a\|x\|_*\leq \|x\|_.\leq b \|x\|_*$ 
