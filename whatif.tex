...
\paragraph{\large Causal Survival Analysis} time-to-event data
...
\paragraph{\large Influence Curve} \citep{hampel1974influence, van2011targeted, zepeda2022delta}
...
\item functional $\phi$ (function of functions): $\exists$ $\partial_v\phi(\theta)$ \& coincides with one-sided directional (Hadamard) derivatives ($=\nabla \phi(\theta)^Tv$)
\end{itemize}

\noindent first order Taylor \textcolor{gray}{(rearranged)}: $\phi(\hat{\theta}_n) \overset{\mathbin{\textcolor{gray}{-}}}{\approx} \phi(\theta) \overset{\mathbin{\textcolor{gray}{\approx}}}{+} \partial_{v:=\hat{\theta}-\theta}\phi(\theta)$
