...
include swigs from 7.5 and that one technical point (?)

more on SWIGS p 242ff



\paragraph{confounding} bias due to common causes of $A$, $Y$ not in $L$

chapter 7

\paragraph{selection bias} bias via conditioning on common effects
...
\noindent \textbf{third step:} use empirical distribution for $Q_{W,0}$ in a substitution estimator: $\psi_n^{TMLE} = \frac{1}{n}\sum_{i=1}^n \left[\bar{Q}^1_n(1,W_i) -  \bar{Q}^1_n(0,W_i)\right] $ 
