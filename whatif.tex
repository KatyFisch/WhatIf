...
\noindent \textit{function} $f:A\to B$ mapping $f(x)=...$, domain = $A$, range $\subseteq B$
``1-1'' if $a_1\neq a_2 \Rightarrow f(a_1)\neq f(a_2)$
``onto'' if $\forall b\in B \exists a\in A: f(a)=b$

\noindent \textit{Cardinality} $A\sim B$ if $\exists f:A\to B$, where $f$ is 1-1 and onto

\noindent \textit{triangle inequality} $|a+b| \leq |a| + |b|$

\noindent \textit{Axiom of Completeness} Every nonempty set of real numbers that is bounded above has a least upper bound

\noindent set $A\subseteq A$ \textit{bound} upper/lower $\exists b\in \mathbb{R}$ s.t.\ $a\leq b$/$b\leq a$ $\forall a\in A$

\noindent \textit{least upper/greatest lower bound} $s \in \mathbb{R}$ s.t.\ $s$ is l/u bound \& $\forall$ u/l bounds $b$: $s\leq u$/$s\geq l$; is the same as supremum/infimum

\noindent \textit{Nested Interval Property} Let $I_n =\left[a_n,b_n\right]=\left\{x\in\mathbb{R}:a_n{\leq} x {\leq} b_n\right\}$, where $n\in \mathbb{N}$ and $I_1\supseteq I_2 \supseteq I_3 ...$, then $\cap_{n=1}^\infty I_n \neq 0$

\noindent \textit{Archimedian Property} $\forall x \in \mathbb{R} \exists n\in\mathbb{N}: x<n$ \& $\forall y>0 \exists n\in\mathbb{N}: 1/n<y$

\noindent \textit{Density of Q in R} $\forall a,b\in\mathbb{R}: \exists r\in \mathbb{Q}: a<r<b$
