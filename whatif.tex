...
components: precise definition of $Y^a$ via specifying $a$ (no different verions of treatment, sufficiently well-defined interventions may not even be possible (effect of DNA before it was discovered), relies on expert consensus); linkage of counterfactuals to data (intervention $a$ must be seen in data) 
 
 

\noindent \textbf{positivity} $\mathrm{Pr}\left[A=a|L=l\right] >0 \,\,\, \forall \, l \text{ with } \mathrm{Pr}\left[L=l\right]>0$

positivity can sometimes be empirically verified


\noindent \textbf{exchangeability} $Y^a \indep A$

(conditionally) randomized = conditionally exchangeable $Y^a \indep A|L$


no guarantee: without randomization unmeasured confounders $U$ may exist
...
\begin{mdframed}[style=MyFrame,nobreak=false, innerleftmargin=2pt, innerrightmargin=2pt]
